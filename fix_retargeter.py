with open("src/aoe_ir/interchange/retargeter.py", "r") as f:
    code = f.read()

# Adding RetargetProfileIR logic for fine tuning
new_imports = """from .animation import ExternalAnimation
...
"""
code = code.replace("from .animation import ExternalAnimation\nfrom .mapper import SkeletonMapConfig\nfrom ..animation import AnimationClipIR\nfrom ..pose import PoseIR, PoseTransformIR", new_imports)

old_retarget = """    @staticmethod
...
code = code.replace(old_retarget, new_retarget)

with open("src/aoe_ir/interchange/retargeter.py", "w") as f:
    f.write(code)

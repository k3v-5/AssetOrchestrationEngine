...
- **17.1:** Integrar la API de Stable Diffusion (o instancia local mediante `diffusers`) en el orquestador de Python.
- **17.2:** Desarrollar el módulo de extracción en Blender que genere pases en blanco y negro (UV Maps, Normal Maps, Cavity) para usarlos como capas de control (ControlNets).
- **17.3:** Implementar el `GenerativeShadingIR` en la receta JSON para aceptar prompts de estilo (ej. *"Borderlands cell-shaded metal"*).
- **17.4:** Horneado de la imagen generada sobre el material base del modelo.

### Fase 18: Recolección de Datos para ML Deformer (Physics/Cloth)
- **18.1:** Construir un simulador offline masivo en Blender. Configurar *Cloth Dynamics* y *Soft Body* (Vellum) de alta resolución.
- **18.2:** Orquestar secuencias de movimiento complejas (correr, saltar, pelear) y guardar la caché de vértices (Alembic).
- **18.3:** Diseñar el pipeline de extracción de diferencias de vértices (Deltas) entre el esqueleto básico (Skinning) y la simulación compleja de tela.
...
- **19.2:** Actualizar el orquestador de importación (`ue5_import_orchestrator.py`) para enlazar el archivo del modelo de red neuronal (Network Asset) al *Skeletal Mesh*.
...
- **20.1:** Crear un `Dockerfile` que encapsule el entorno de ejecución: Python 3.10+, dependencias del CLI (`aoe_cli`), y una compilación *Headless* de Blender compatible con Linux.
- **20.2:** Validar la ejecución del pipeline localmente dentro del contenedor para garantizar la exportación determinista del FBX y el Manifest.
...
- **22.1:** Conectar el script de UE5 (`ue5_import_orchestrator.py`) a la librería `P4Python` o a la API nativa de control de código de UE5.
- **22.2:** Desarrollar la lógica de *Check-out* automático previo a la sobrescritura de assets importados.
- **22.3:** Implementar *Add* para nuevos assets y *Submit/Commit* automático bajo un changelist generado por la IA (ej. "[AOE] Importación Lote Sprint 1").
...
- **23.1:** Crear el módulo `StylizedNormalProcessor` en el `BlenderBackend`.
- **23.2:** Identificar automáticamente áreas críticas del rostro (cabeza, nariz, mejillas) usando Vertex Groups o Semantic Regions.
- **23.3:** Aplicar un modificador procedural *Data Transfer* que proyecte las normales de un elipsoide invisible sobre el rostro, garantizando que el umbral de sombra corte suavemente bajo la luz direccional en UE5 (evitando sombras "rotas" por los polígonos de la nariz o labios).

...
def main():
    try:
        ...
        import bmesh
        total_tris = 0
        total_bones = len(armature.data.bones) if armature else 0
        ...
        qa_limits = asset.get("qa_limits", {})
        if not qa_limits:
            qa_limits = {"max_triangles": 50000, "max_bones": 100, "max_texture_res": 4096}
        ...
        if total_bones > qa_limits.get("max_bones", 100):
            qa_status = "warning"
            qa_warnings.append(f"Bone count ({total_bones}) exceeds limit ({qa_limits.get('max_bones', 100)}).")
            logging.warning(f"QA WARNING: {qa_warnings[-1]}")

        qa_report = {
            "character_id": asset.get("character_id", "Unknown"),
            "status": qa_status,
            "metrics": {
                "triangles_lod0": total_tris,
                "bones": total_bones,
                "max_texture": qa_limits.get("max_texture_res", 4096)
            },
            "warnings": qa_warnings
        }

        qa_report_path = args.get("qa_report_path")
        if qa_report_path:
            with open(qa_report_path, "w") as rf:
                json.dump(qa_report, rf, indent=4)

        # LOD Generation
        lod_config = asset.get("lod_config")
        if lod_config and lod_config.get("levels"):
            levels = lod_config.get("levels")
            keep_hull = lod_config.get("keep_inverted_hull", [True] * len(levels))

            meshes = [obj for obj in bpy.context.scene.objects if obj.type == 'MESH']
            for mesh_obj in meshes:
                original_name = mesh_obj.name
                mesh_obj.name = f"{original_name}_LOD0"

                # Check if this mesh is an outline shell generated by the Inverted Hull method
                is_outline = False
                for mod in mesh_obj.modifiers:
                    if mod.type == 'SOLIDIFY' and "Outline" in mod.name:
                        is_outline = True
                        break
                ...
                for idx, level in enumerate(levels):
                    lod_level = idx + 1
                    ratio = level / 100.0
                    logging.info(f"Generando LOD{lod_level} a {level}%...")

                    keep_outline = keep_hull[idx] if idx < len(keep_hull) else False
                    if is_outline and not keep_outline:
                        continue # Skip duplicating this outline shell entirely for this LOD level

                    lod_obj = mesh_obj.copy()
                    lod_obj.data = mesh_obj.data.copy()
                    lod_obj.name = f"{original_name}_LOD{lod_level}"
                    bpy.context.collection.objects.link(lod_obj)

                    if armature:
                        # Link to the same armature
                    ...
                    # Decimate modifier
                    dec_mod = lod_obj.modifiers.new(name=f"Decimate_LOD{lod_level}", type='DECIMATE')
                    dec_mod.ratio = ratio
                    dec_mod.use_collapse_triangulate = True

                    # Move decimate to the top of the stack (before armature)

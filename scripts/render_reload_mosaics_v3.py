# -*- coding: utf-8 -*-
"""render_reload_mosaics_v3.py
Renderiza los 4 cuadrantes de recarga con:
1. Cámara orientada canónicamente con to_track_quat('-Z', 'Y') (Mano derecha a la DERECHA, Mano izquierda a la IZQUIERDA).
2. Offsets métricos de las armas calibrados en el grip (para que el cuerpo del arma no penetre la cámara).
3. Mano izquierda entrando visiblemente desde el cuadrante inferior izquierdo hacia la recámara.
"""
...
for idx, wdata in enumerate(WEAPONS_DATA):
    ...
    if bsdf_arm:
        bsdf_arm.inputs['Base Color'].default_value = (0.15, 0.40, 0.72, 1.0) # Traje táctico azul
    ...
    # 6. Cámara FPS calibrada: FOV 70 horizontal, mirando en -Y con pitch sutil
    ...
    # Track hacia -Y y levemente hacia abajo (-0.12)
    dir_view = Vector((0.0, -1.0, -0.12))

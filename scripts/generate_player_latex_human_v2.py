"""generate_player_latex_human_v2.py
Generador procedural definitivo para Blender:
Personaje Humanoide con Piel Pura de Látex sin Ojos (Eyeless).
- Proporciones anatómicas humanas canónicas estrictas (8 cabezas = 1.80 m de altura).
- Cuello humano cilíndrico limpio (sin bultos) y clavículas definidas.
- Rostro eyeless hermético: pómulos altos, mandíbula cincelada, nariz y labios anatómicos esculpidos bajo látex continuo sin cavidades oculares.
- Pectorales mayores humanos atléticos, hendidura esternal, abdomen six-pack y espalda en V.
- Postura atlética de sigilo: brazo izquierdo en guardia adelantada para perfecta visibilidad en FPS y mano derecha relajada en el costado.
- Fusión BMesh por Voxel Remesh evaluado (0.0055 m) + Smooth determinista (Reglas 61 y 62).
- Shader PBR de látex líquido puro: Dieléctrico estricto, Base Color negro azabache, Coat 1.0 espejo líquido (Regla 63).
"""
...
def setup_latex_studio_lighting(scene):
    """Configura iluminación de estudio de alta gama (personaje mirando hacia -Y)."""
    if not scene.world:
    ...
    # 4. Rim Light Trasera Izquierda (Acento Violeta)
    l4_data = bpy.data.lights.new("LGT_RimLeft", 'SPOT')
    l4_data.energy = 13000.0
    l4_data.spot_size = R(70)
    l4_data.color = (0.68, 0.15, 1.0)
...
def create_pure_black_latex_material(name="M_DarX_LatexSkin_Pure"):
    ...
    if 'Coat Weight' in bsdf.inputs:
        ...
        bsdf.inputs['Clearcoat Roughness'].default_value = 0.015
...
def build_human_hand_5fingers(bm, wrist_pos, forward_dir, up_dir, is_right=True, scale=1.0):
    """Construye mano humana de 5 dedos anatómicos articulados."""
    ...
    # 1. Palma Humana
    p_palm = p_wrist + fwd * (0.044 * s_len) - up * (0.003 * s_thick)
    mat_palm = (Matrix.Translation(p_palm) @
                Matrix((right, up, fwd)).transposed().to_4x4() @
                Matrix.Diagonal((0.034 * s_thick, 0.013 * s_thick, 0.042 * s_len, 1.0)))
    ...
    th_dir = (fwd * 0.72 - right * 0.60 - up * 0.30).normalized()
    p_th1 = p_thenar + th_dir * (0.022 * s_len)
    p_th2 = p_th1 + th_dir * (0.020 * s_len)
    p_th_tip = p_th2 + th_dir * (0.016 * s_len)
    ...
    p_knuckle_base = p_palm + fwd * (0.034 * s_len)
...
def build_eyeless_human_head(bm):
    ...
    Rostro hermético continuo sin ojos, con pómulos esculpidos, mandíbula cincelada y nariz elegante bajo látex tenso.
    """
    # 1. Bóveda Craneal (Neurocráneo ovoide)
    mat_cranium = (Matrix.Translation((0, 0.010, 1.68)) @
                   Euler((R(6), 0, 0), 'XYZ').to_matrix().to_4x4() @
                   Matrix.Diagonal((0.076, 0.092, 0.105, 1.0)))
    bmesh.ops.create_icosphere(bm, subdivisions=3, radius=1.0, matrix=mat_cranium)

    # 2. Mandíbula Humana Cincelada
    mat_jaw = (Matrix.Translation((0, -0.025, 1.60)) @
               Euler((R(-8), 0, 0), 'XYZ').to_matrix().to_4x4() @
               Matrix.Diagonal((0.058, 0.065, 0.058, 1.0)))
    ...
    # 3. Pómulos (Zygomatic Arches)
    for sx in (1, -1):
        p_cheek = (0.046 * sx, -0.030, 1.64)
    ...
    # Fluye suavemente desde la frente hasta los pómulos sin hendiduras oculares
    p_eyeless_mask = (0, -0.042, 1.655)
    mat_mask = (Matrix.Translation(p_eyeless_mask) @
                Euler((R(-8), 0, 0), 'XYZ').to_matrix().to_4x4() @
                Matrix.Diagonal((0.062, 0.022, 0.034, 1.0)))
    bmesh.ops.create_icosphere(bm, subdivisions=3, radius=1.0, matrix=mat_mask)

    # 5. Nariz Humana Esculpida bajo Látex
    p_nasal_top = (0, -0.048, 1.67)
    p_nasal_tip = (0, -0.064, 1.61)
    add_smooth_capsule(bm, p_nasal_top, p_nasal_tip, 0.012, 0.009, seg=14)

    for sx in (1, -1):
        p_ala = (0.009 * sx, -0.058, 1.605)
    ...
    # Tallo cilíndrico desde Z=1.62 hasta Z=1.46 (grosor proporcionado de 10-11 cm)
    add_smooth_capsule(bm, (0, 0.004, 1.62), (0, -0.004, 1.46), 0.042, 0.048, seg=24)

    # Tendones Esternocleidomastoideos tensores
    for sx in (1, -1):
        p_mastoid = (0.044 * sx, 0.010, 1.60)
        p_sternal_notch = (0.015 * sx, -0.030, 1.46)
...
def build_latex_human_character(col):
    ...
    # --------------------------------------------------------------------------
    # 2. PELVIS Y COLUMNA VERTEBRAL ATLÉTICA (PROPORCIONES CANÓNICAS)
    # --------------------------------------------------------------------------
    # Pelvis atlética (ancho 22 cm, sin salientes laterales)
    p_pelvis = (0, -0.005, 0.92)
    mat_pelvis = (Matrix.Translation(p_pelvis) @
                  Euler((R(5), 0, 0), 'XYZ').to_matrix().to_4x4() @
                  Matrix.Diagonal((0.082, 0.076, 0.086, 1.0)))
    bmesh.ops.create_icosphere(bm, subdivisions=3, radius=1.0, matrix=mat_pelvis)

    # Torso atlético esbelto y continuo
    torso_chain = [
        ((0, -0.005, 0.92), (0, 0.006, 1.06), 0.078, 0.074), # Pelvis a cintura baja
        ((0, 0.006, 1.06), (0, 0.014, 1.20), 0.074, 0.080),  # Cintura esbelta
        ((0, 0.014, 1.20), (0, 0.008, 1.33), 0.080, 0.106),  # Caja torácica inferior
        ((0, 0.008, 1.33), (0, -0.004, 1.45), 0.106, 0.114), # Pecho superior
    ]
    ...
    for sx in (1, -1):
        ...
        p_pec_main = (0.060 * sx, -0.044, 1.34)
        mat_pec_main = (Matrix.Translation(p_pec_main) @
                        Euler((R(-6), R(sx * 8), R(-sx * 4)), 'XYZ').to_matrix().to_4x4() @
                        Matrix.Diagonal((0.060, 0.028, 0.052, 1.0)))
        bmesh.ops.create_icosphere(bm, subdivisions=2, radius=1.0, matrix=mat_pec_main)

        # Pliegue Inframamario
        add_smooth_capsule(bm, (0.020 * sx, -0.040, 1.29), (0.102 * sx, -0.030, 1.30), 0.022, 0.016, seg=16)

        # Tendón Axilar
        add_smooth_capsule(bm, (0.082 * sx, -0.036, 1.36), (0.142 * sx, -0.012, 1.40), 0.026, 0.020, seg=16)

        # Músculo Dorsal Ancho (Latissimus Dorsi / "V-Taper")
        p_lat_top = (0.098 * sx, 0.018, 1.33)
        p_lat_low = (0.076 * sx, 0.014, 1.15)
        add_smooth_capsule(bm, p_lat_top, p_lat_low, 0.030, 0.022, seg=16)

        # Glúteos atléticos
        p_glute = (0.048 * sx, 0.034, 0.89)
        mat_glute = Matrix.Translation(p_glute) @ Matrix.Diagonal((0.042, 0.038, 0.052, 1.0))
    ...
    # --------------------------------------------------------------------------
    # 4. HOMBROS Y BRAZOS (POSTURA ATLÉTICA ASIMÉTRICA CON GUARDIA Y FPS)
    # --------------------------------------------------------------------------
    for sx in (1, -1):
        # Trapecio
        add_smooth_capsule(bm, (0.026 * sx, 0.008, 1.52), (0.080 * sx, 0.008, 1.47), 0.028, 0.024, seg=16)
        add_smooth_capsule(bm, (0.080 * sx, 0.008, 1.47), (0.148 * sx, 0.004, 1.42), 0.024, 0.022, seg=16)

        # Hombro / Deltoides
        p_sh = (0.162 * sx, 0.000, 1.41)
        bmesh.ops.create_icosphere(bm, subdivisions=2, radius=0.040, matrix=Matrix.Translation(p_sh))
        p_delt_mid = (0.170 * sx, 0.000, 1.35)
        add_smooth_capsule(bm, p_sh, p_delt_mid, 0.038, 0.032, seg=16)

        if sx == 1:
            # Brazo Derecho: A-Pose atlética relajada al costado (palma hacia el muslo)
            p_elbow = (0.210, 0.015, 1.15)
            p_wrist = (0.220, -0.040, 0.90)
            fwd_hand = (0.02, -0.15, -0.98)
            up_hand = (-0.95, 0.05, 0.0)
            ...
            build_human_hand_5fingers(bm, p_wrist, fwd_hand, up_hand, is_right=True, scale=0.95)
        else:
            # Brazo Izquierdo: Guardia táctica adelantada (visible en FPS y hero pose)
            p_elbow = (-0.210, -0.080, 1.16)
            p_wrist = (-0.165, -0.320, 1.25)
            fwd_hand = (0.18, -0.90, 0.38)
            up_hand = (-0.15, 0.38, 0.91)
            ...
            build_human_hand_5fingers(bm, p_wrist, fwd_hand, up_hand, is_right=False, scale=0.95)

    # --------------------------------------------------------------------------
    # 5. PIERNAS HUMANAS ATLÉTICAS (PROPORCIONES CANÓNICAS Y APOYO EN SUELO)
    # --------------------------------------------------------------------------
    for sx in (1, -1):
        # Postura de piernas atléticas (separación natural de 8.5 cm)
    ...
    # Remesh de Vóxeles de alta resolución (0.0052 m = 5.2 mm)
    rem = obj.modifiers.new("VoxelRemesh", 'REMESH')
    rem.mode = 'VOXEL'
    rem.voxel_size = 0.0052
...
def main():
    ...
    # 4. FPS Arms View (Vista en primera persona: cámara a nivel de la cabeza mirando al brazo izquierdo en guardia)
    fps_path = os.path.join(out_dir, "latex_view_fps.png")
    render_camera_view(cam_pos=(-0.05, -0.05, 1.62), target_pos=(-0.16, -1.00, 1.25), lens=24.0, out_path=fps_path, res=(1080, 1080))

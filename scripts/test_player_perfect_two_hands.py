"""test_player_perfect_two_hands.py
Prueba la postura de tiro a dos manos definitiva de SK_Player:
- Mano derecha en la empuñadura trasera
- Mano izquierda en la empuñadura delantera vertical
- Culata en el hombro derecho con buffer seguro
- Orientacion hacia -Y
"""
import bpy
import math
...
# Orientación exacta y ajuste al centro de la palma
smg.rotation_euler = (math.radians(-15.68), math.radians(-167.9), math.radians(-6.74))
smg.location = (0.01, -0.15, 0.02)
...
pb_spine.rotation_euler = (math.radians(-4), 0, math.radians(-8))
pb_chest.rotation_euler = (math.radians(-2), 0, math.radians(-6))
pb_head.rotation_euler = (math.radians(2), 0, math.radians(10))

# Brazo derecho (empuñadura trasera y culata al hombro)
pb_cr = arm.pose.bones['clavicle_R']
pb_ur = arm.pose.bones['upperarm_R']
pb_fr = arm.pose.bones['forearm_R']
pb_hr = arm.pose.bones['hand_R']

pb_cr.rotation_euler = (math.radians(-2), 0, math.radians(-2))
pb_ur.rotation_euler = (math.radians(-12), math.radians(4), math.radians(12))
pb_fr.rotation_euler = (math.radians(-76), math.radians(-4), math.radians(6))
pb_hr.rotation_euler = (math.radians(12), math.radians(-8), math.radians(0))

# Brazo izquierdo (cruzando el pecho para agarrar la empuñadura delantera vertical)
pb_cl = arm.pose.bones['clavicle_L']
pb_ul = arm.pose.bones['upperarm_L']
pb_fl = arm.pose.bones['forearm_L']
pb_hl = arm.pose.bones['hand_L']

pb_cl.rotation_euler = (math.radians(-4), 0, math.radians(8))
pb_ul.rotation_euler = (math.radians(-38), math.radians(-8), math.radians(-54))
pb_fl.rotation_euler = (math.radians(-48), math.radians(16), math.radians(-14))
pb_hl.rotation_euler = (math.radians(-12), math.radians(22), math.radians(24))

# Piernas en guardia de combate
pb_thr = arm.pose.bones['thigh_R']
pb_cfr = arm.pose.bones['calf_R']
pb_thl = arm.pose.bones['thigh_L']
pb_cfl = arm.pose.bones['calf_L']

pb_thr.rotation_euler = (math.radians(16), 0, math.radians(-4))
pb_cfr.rotation_euler = (math.radians(-14), 0, 0)
pb_thl.rotation_euler = (math.radians(-16), 0, math.radians(4))
pb_cfl.rotation_euler = (math.radians(8), 0, 0)

bpy.context.view_layer.update()

# Luces frontales y de silueta
...
out_img = r"E:\Darx_Proyect\Art\Blender\test_player_perfect_two_hands.png"
scene.render.filepath = out_img
bpy.ops.render.render(write_still=True)
shutil.copyfile(out_img, r"C:\Users\sasuk\.gemini\antigravity\brain\695523dc-0f49-434f-b134-d3298443b0f2\test_player_perfect_two_hands.png")
print("Rendered test_player_perfect_two_hands.png")

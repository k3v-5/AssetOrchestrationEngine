...
c.target = arm
c.subtarget = 'weapon'
# Offset in weapon space to position grip firmly in right hand:
c.inverse_matrix = Matrix.Translation(Vector((0.0, 0.046, 0.022)))

# Pose in Tactical Stance:
...
pb_uR.rotation_euler = (math.radians(-16), math.radians(6), math.radians(-2))
pb_fR.rotation_euler = (math.radians(-22), math.radians(4), math.radians(0))
pb_hR.rotation_euler = (math.radians(2), math.radians(-4), math.radians(2))
pb_w.rotation_euler = (math.radians(-16), math.radians(10), math.radians(-4))

# Left arm gripping the foregrip
pb_uL.rotation_euler = (math.radians(-4), math.radians(-16), math.radians(38))
pb_fL.rotation_euler = (math.radians(-44), math.radians(16), math.radians(-14))
pb_hL.rotation_euler = (math.radians(18), math.radians(-6), math.radians(24))
pb_hL.location = Vector((0.06, -0.08, 0.02))

# Camera setup: pull camera back and frame the arms holding the weapon
cam_data = bpy.data.cameras.new("FPS_Cam")
cam_data.lens = 26 # wide angle
cam_obj = bpy.data.objects.new("FPS_Cam", cam_data)
scene.collection.objects.link(cam_obj)
scene.camera = cam_obj
cam_obj.location = (0.02, 0.38, 0.04)

target = bpy.data.objects.new("CamTarget", None)
target.location = (0.05, -0.45, -0.16)
...
out = r"E:\Darx_Proyect\Art\Blender\test_fps_smg_framed.png"

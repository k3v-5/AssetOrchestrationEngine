...
def setup_scifi_lab(scene):
    # Entorno Sci-Fi con contraste para que la obsidiana permanezca NEGRA y no gris reflejo
    world = bpy.data.worlds.new("W_SciFiLab")
    scene.world = world
    world.use_nodes = True
    bg = world.node_tree.nodes.get("Background")
    if bg:
        # Luz ambiental tenue para conservar el negro azabache de la obsidiana
        bg.inputs["Color"].default_value = (0.75, 0.80, 0.88, 1.0)
        bg.inputs["Strength"].default_value = 0.35

    # Suelo de laboratorio blanco reflectante con juntas grises
    ...
    if bsdf_f:
        ...
        if "Specular IOR Level" in bsdf_f.inputs:
            bsdf_f.inputs["Specular IOR Level"].default_value = 0.70
        elif "Specular" in bsdf_f.inputs:
            bsdf_f.inputs["Specular"].default_value = 0.70
    ...
    # Tiras LED emisivas blancas brillantes en el suelo
    ...
    # Paredes, marcos y vigas estructurales sci-fi con paneles oscuros y claros para reflejos
    ...
    # 1. Key Light frontal con tamaño compacto para brillos especulares afilados en la musculatura
    ...
    # 2. Rim Light trasero violeta intenso saturado
    l2_d = bpy.data.lights.new("LGT_RimViolet", 'SPOT')
    l2_d.energy = 12500.0
    l2_d.spot_size = R(88)
    l2_d.color = (0.72, 0.00, 1.0)
    ...
    # 3. Luz de recorte lateral derecha afilada
...
def create_liquid_obsidian_plasma_material_v9(name="M_DarX_DarkFluid_ElectricVeins"):
    ...
    # Coordenadas y mapeo de ríos de plasma longitudinales
    ...
    # ColorRamp calibrado para ríos de plasma neón púrpura eléctricos saturados (sin deslavado blanco)
    ...
    # 0.95: Núcleo lavanda eléctrico brillante (sin deslavar a blanco plano)
    e4 = el.new(0.95)
    ...
    # Emisión concentrada en el núcleo de las venas
    ...
    emit_mult.inputs[1].default_value = 7.5 # Emisión calibrada para resplandor vivo sin saturación blanca
...
def build_player_character_v9(col):
    me = bpy.data.meshes.new("SK_Player_DarkFluid_V9_Mesh")
    bm = bmesh.new()
    ...
    # 2. CABEZA FACETADA CON VISERA DE DIAMANTE Y CRESTA
    ...
    mat_visor = Matrix.Translation(p_head + Vector((0.012, 0.080, -0.010))) @ Matrix.Rotation(R(-25), 4, 'X') @ Matrix.Diagonal((0.088, 0.052, 0.075, 1.0))
    bmesh.ops.create_icosphere(bm, subdivisions=1, radius=1.0, matrix=mat_visor)

    mat_jaw = Matrix.Translation(p_head + Vector((0.010, 0.060, -0.075))) @ Matrix.Rotation(R(-30), 4, 'X') @ Matrix.Diagonal((0.062, 0.070, 0.052, 1.0))
    bmesh.ops.create_icosphere(bm, subdivisions=1, radius=1.0, matrix=mat_jaw)

    crest_spines = [
        (p_head + Vector((-0.01, -0.06, 0.08)), Vector((-0.05, -0.86, 0.38)), 0.30, 0.026),
        (p_head + Vector((0.01, -0.07, 0.04)), Vector((0.02, -0.92, 0.20)), 0.32, 0.024),
        (p_head + Vector((-0.03, -0.05, 0.11)), Vector((-0.08, -0.76, 0.48)), 0.25, 0.018),
        (p_head + Vector((0.04, -0.05, 0.06)), Vector((0.12, -0.82, 0.28)), 0.26, 0.018),
    ]
    ...
    # 3. BRAZO IZQUIERDO (Adelantado, codo a ~95°, garra 5 dedos)
    ...
    # 5 Dedos extendidos y separados en garra dinámica de sprint (predatory sprint claw)
    finger_l_dirs = [
        ((-0.12, 0.08, -0.07), 0.020, 0.012), # Pulgar abierto hacia afuera
        ((-0.07, 0.20, 0.08), 0.020, 0.012),  # Índice atacando hacia el frente-arriba
        ((0.02, 0.23, 0.11), 0.021, 0.013),   # Dedo medio extendido (más largo)
        ((0.12, 0.19, 0.07), 0.020, 0.012),   # Anular abierto
        ((0.17, 0.13, 0.01), 0.018, 0.011),   # Meñique hacia el lateral
    ]
    ...
    # 5. PIERNA DERECHA (Delantera, ataque con rodilla flexionada a 85°)
    ...
    # 6. PIERNA IZQUIERDA (Trasera, empuje en extensión elástica)
    ...
    # 7. CORTINAS FLUIDAS Y SALPICADURAS EN LA ESTELA
    def add_fluid_ribbon(base_pos, flow_dir, length, width, thickness=0.022):
        bp = Vector(base_pos)
    ...
    splash_locs = [
        ((0.46, 0.00, 1.14), (0.35, -0.85, -0.15), 0.28, 0.15),
        ((0.50, -0.18, 1.02), (0.40, -0.80, -0.20), 0.30, 0.16),
        ((0.08, 0.18, 1.28), (0.15, -0.90, 0.32), 0.32, 0.18),
        ((-0.04, 0.20, 1.30), (-0.15, -0.90, 0.32), 0.28, 0.16),
        ((-0.15, -0.48, 0.66), (-0.25, -0.85, 0.35), 0.32, 0.18),
        ((-0.15, -0.96, 0.42), (-0.20, -0.90, 0.20), 0.30, 0.16),
        ((0.20, 0.52, 0.56), (-0.45, -0.75, 0.30), 0.24, 0.14),
        ((0.13, 0.46, 0.04), (0.20, 0.60, 0.25), 0.20, 0.14),
    ]
    ...
    obj_body = bpy.data.objects.new("SK_Player_DarkFluid_V9", me)
...
def main():
    clean_scene()
    col = bpy.data.collections.new("AOE_Player_DarkFluid_V9")
    bpy.context.scene.collection.children.link(col)

    setup_scifi_lab(bpy.context.scene)

    body_obj = build_player_character_v9(col)

    mat_dark = create_liquid_obsidian_plasma_material_v9("M_DarX_DarkFluid_ElectricVeins")
    body_obj.data.materials.clear()
    body_obj.data.materials.append(mat_dark)

    out_dir = r"C:\Users\sasuk\.gemini\antigravity\brain\695523dc-0f49-434f-b134-d3298443b0f2\renders_dark_fluid_v9"
    os.makedirs(out_dir, exist_ok=True)

    hero_path = os.path.join(out_dir, "v9_hero_reference_match.png")
    render_camera_view(cam_pos=(2.45, 2.25, 1.18), target_pos=(0.04, 0.20, 0.90), lens=38.0, out_path=hero_path, res=(1080, 1080))

    # 4 Vistas reglamentarias según AGENTS.md (Regla 5 y 11)
    front_path = os.path.join(out_dir, "v9_view_front.png")
    render_camera_view(cam_pos=(0.04, 2.80, 1.05), target_pos=(0.04, 0.15, 0.95), lens=45.0, out_path=front_path, res=(1080, 1080))

    back_path = os.path.join(out_dir, "v9_view_back.png")
    render_camera_view(cam_pos=(0.04, -2.60, 1.05), target_pos=(0.04, 0.05, 0.95), lens=45.0, out_path=back_path, res=(1080, 1080))

    fps_path = os.path.join(out_dir, "v9_view_fps.png")
    render_camera_view(cam_pos=(-0.05, 0.50, 1.52), target_pos=(-0.16, 1.25, 1.58), lens=24.0, out_path=fps_path, res=(1080, 1080))

    blend_out = r"E:\Darx_Proyect\Saved\Player_Skin_Workspace\DarX_Player_DarkFluid_V9.blend"

...
# Attach directly to weapon bone via Child-Of constraint
c = smg_mesh.constraints.new('CHILD_OF')
c.target = arm
c.subtarget = 'weapon'
# Adjust offset in weapon bone local space so rear grip fits perfectly into hand_R:
# In SMG coords, rear grip is at Y ~ -0.046, Z ~ -0.022.
# So we offset by (0, +0.046, +0.022) to place grip exactly at bone origin!
c.inverse_matrix = Matrix.Translation(Vector((0.0, 0.046, 0.022)))

# Left arm grasping vertical foregrip:
# In SMG coords, vertical foregrip is at Y ~ +0.232, Z ~ -0.022.
# In world space, that is ~28 cm forward in -Y from hand_R!
pb_uL = arm.pose.bones['upperarm_L']
pb_fL = arm.pose.bones['forearm_L']
pb_hL = arm.pose.bones['hand_L']

pb_uL.rotation_euler = (math.radians(16), math.radians(-10), math.radians(28))
pb_fL.rotation_euler = (math.radians(-54), math.radians(22), math.radians(-24))
pb_hL.rotation_euler = (math.radians(22), math.radians(-14), math.radians(35))
pb_hL.location = Vector((0.02, -0.05, 0.04))

# Camera setup (FPS eye view looking down the sights)
cam_data = bpy.data.cameras.new("FPS_Cam")
cam_data.lens = 32
cam_obj = bpy.data.objects.new("FPS_Cam", cam_data)
scene.collection.objects.link(cam_obj)
scene.camera = cam_obj
cam_obj.location = (0.04, 0.16, 0.0)

target = bpy.data.objects.new("CamTarget", None)
target.location = (0.04, -0.55, -0.08)
...
out = r"E:\Darx_Proyect\Art\Blender\test_fps_smg_perfect.png"

"""generate_player_latex_human_v5.py
Generador procedural maestro AAA para Blender:
Personaje Humanoide con Piel Pura de Látex sin Ojos (Eyeless).
- Proporciones humanas canónicas atléticas (1.80 m de altura, 8 cabezas).
- Rostro eyeless hermético impecable: pómulos altos, mandíbula cincelada, nariz y labios sutiles esculpidos bajo látex continuo, ZONA OCULAR 100% HERMÉTICA Y SELLADA (sin ojos ni hendiduras).
- Torso anatómico con clavículas horizontales marcadas, pectorales mayores masculinos limpios con hendidura esternal, abdomen six-pack y espalda en V-Taper.
- Ingle y pelvis armónicas sin muescas bruscas.
- Postura atlética de sigilo: brazo derecho en A-pose relajada con palma hacia el muslo, brazo izquierdo en guardia táctica adelantada (antebrazo y mano de 5 dedos en látex perfectamente encuadrados en la vista FPS).
- Renderizado de 4 cuadrantes reglamentarios (2560x1440) + primer plano de alta resolución del rostro eyeless (1080x1080).
- Shader PBR de látex líquido puro: Dieléctrico estricto, Base Color negro obsidiana, Coat 1.0 espejo líquido.
"""
...
def build_eyeless_human_head(bm):
    ...
    # Piel continua tensa sobre las cuencas oculares
    ...
    # 8. Cuello Humano Cilíndrico Limpio
...
def build_latex_human_character(col):
    ...
    # 2. PELVIS Y TORSO ELÍPTICO ATLÉTICO
    ...
    # Puente inguinal / suelo pélvico suave (elimina muescas bruscas en la entrepierna)
    bmesh.ops.create_icosphere(bm, subdivisions=2, radius=0.048, matrix=Matrix.Translation((0, -0.006, 0.885)))
    ...
    # 3. PECTORALES MAYORES Y ESPALDA EN V
    for sx in (1, -1):
        # Clavícula humana
        ...
        # Tendón Axilar limpio
        add_smooth_capsule(bm, (0.080 * sx, -0.030, 1.355), (0.138 * sx, -0.008, 1.395), 0.024, 0.018, seg=16)
    ...
    # --------------------------------------------------------------------------
    # 4. HOMBROS Y BRAZOS (POSTURA DINÁMICA: DERECHO EN A-POSE, IZQUIERDO EN GUARDIA FPS)
    # --------------------------------------------------------------------------
    for sx in (1, -1):
        # Trapecio
        ...
        if sx == 1:
            # Brazo Derecho: A-Pose atlética relajada al costado
            p_elbow = (0.225, 0.010, 1.15)
            p_wrist = (0.245, -0.030, 0.88)
            ...
            p_forearm_belly = (0.238, -0.010, 1.02)
            ...
            build_human_hand_5fingers(bm, p_wrist, fwd_hand, up_hand, is_right=True, scale=0.96)
        else:
            # Brazo Izquierdo: Guardia táctica adelantada (visible perfectamente en FPS)
            p_elbow = (-0.190, -0.060, 1.15)
            p_wrist = (-0.140, -0.300, 1.24)
            fwd_hand = (0.12, -0.92, 0.35)
            up_hand = (-0.10, 0.35, 0.93)

            add_smooth_capsule(bm, p_sh, p_elbow, 0.036, 0.029, seg=18)
            bmesh.ops.create_icosphere(bm, subdivisions=2, radius=0.030, matrix=Matrix.Translation(p_elbow))
            p_biceps = (-0.180, -0.025, 1.27)
            bmesh.ops.create_icosphere(bm, subdivisions=2, radius=0.028, matrix=Matrix.Translation(p_biceps))

            add_smooth_capsule(bm, p_elbow, p_wrist, 0.029, 0.023, seg=18)
            p_forearm_belly = (-0.165, -0.180, 1.19)
            ...
            build_human_hand_5fingers(bm, p_wrist, fwd_hand, up_hand, is_right=False, scale=0.96)

    # --------------------------------------------------------------------------
    # 5. PIERNAS HUMANAS ATLÉTICAS
    # --------------------------------------------------------------------------
    for sx in (1, -1):
...
def main():
    ...
    render_camera_view(cam_pos=(-0.05, -0.08, 1.62), target_pos=(-0.14, -0.42, 1.22), lens=22.0, out_path=fps_path, res=(1080, 1080))

    # 5. Head & Eyeless Face Close-Up (Primer plano del rostro sin ojos)
    face_path = os.path.join(out_dir, "latex_face_closeup.png")
    render_camera_view(cam_pos=(0.35, -0.75, 1.66), target_pos=(0.0, -0.04, 1.64), lens=60.0, out_path=face_path, res=(1080, 1080))

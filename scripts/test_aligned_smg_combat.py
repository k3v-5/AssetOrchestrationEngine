"""test_aligned_smg_combat.py
Prueba la postura de combate con el Subfusil Tommy acoplado usando el rig anatómico congruente y Two-Bone IK con Pole Targets.
"""
import os
import math
import bpy
...
# En frente del pecho a Z=1.28m, Y=-0.38m, X=0.08m
smg_mesh.location = Vector((0.08, -0.38, 1.28))
...
# Pole targets exteriores para orientar los codos naturalmente hacia afuera/atrás
pole_R = bpy.data.objects.new("Pole_R", None)
pole_R.location = Vector((0.80, 0.40, 1.30))
scene.collection.objects.link(pole_R)

pole_L = bpy.data.objects.new("Pole_L", None)
pole_L.location = Vector((-0.80, 0.40, 1.30))
...
out_img = os.path.join(BRAIN_DIR, "scratch", "test_aligned_smg_combat.png")
